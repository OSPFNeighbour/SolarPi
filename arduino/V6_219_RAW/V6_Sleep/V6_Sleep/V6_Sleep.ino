...
float avg_solar_shuntvoltage[AverageOverThisMayLoops];
float avg_solar_busvoltage[AverageOverThisMayLoops];
float avg_solar_current_mA[AverageOverThisMayLoops];
float avg_solar_loadvoltage[AverageOverThisMayLoops];
...
void setup()
{
  ...
  for (int i = 0; i <= AverageOverThisMayLoops; i++) {
    ...
    avg_solar_shuntvoltage[i] = ina219_solar.getShuntVoltage_mV();
    avg_solar_busvoltage[i] = ina219_solar.getBusVoltage_V();
    avg_solar_current_mA[i] = ina219_solar.getCurrent_mA();
    avg_solar_loadvoltage[i] = avg_solar_busvoltage[i] + (avg_solar_shuntvoltage[i] / 1000);
...
void loop()
{
  ...
  avg_solar_shuntvoltage[AvgPosition] = solar_shuntvoltage;
  avg_solar_busvoltage[AvgPosition] = solar_busvoltage;
  avg_solar_current_mA[AvgPosition] = solar_current_mA;
  avg_solar_loadvoltage[AvgPosition] = solar_loadvoltage;
  ...
  if (LoopCounter == 1)
  {
    ...
    lcd.print(" SV:");
    lcd.print(solar_loadvoltage);
    

    Serial.print("|");
    Serial.print("S:");
    Serial.print(average(avg_solar_loadvoltage, AverageOverThisMayLoops));
    Serial.print("|");
    Serial.print("B:");
    ...
    Serial.print("|");
    Serial.print("M:");
    Serial.print(average(avg_solar_current_mA, AverageOverThisMayLoops) * average(avg_solar_loadvoltage, AverageOverThisMayLoops));
    Serial.println("|");
